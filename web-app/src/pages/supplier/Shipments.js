...
const Shipments = () => {
  ...
  useEffect(() => {
    const checkSupplierWallet = async () => {
      if (user?.role_type === 'supplier') {
        try {
          const response = await axios.get(
            `http://localhost:3003/ship/checkSupplierWallet/${user.entity_id}`,
            getAuthHeaders()
          );
          if (response.data.walletAddress === null) {
            setWalletWarning(true); // Устанавливаем предупреждение, если кошелек отсутствует
          }
        } catch (error) {
          console.error('Ошибка при проверке кошелька поставщика:', error);
        }
      }
    };
  ...
  const fetchCompaniesAndSuppliers = async () => {
    ...
    try {
      const response = await axios.get(
        'http://localhost:3003/ship/getEntitiesForShipment',
        getAuthHeaders()
      );
  ...
  return (
    <motion.div 
      ...
      exit="exit"
    >
      {/* Предупреждение о необходимости ввода кошелька */}
      {walletWarning && (
        <div className="mb-6 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
          <p className="text-yellow-800">
            Для продолжения работы необходимо указать кошелек оплаты. Перейдите в{' '}
              ...
              >
                ...
                >
                  ...
                  <form onSubmit={handleCreateShipment} className="space-y-6">
                    {user.role_type === 'supplier' && (<motion.div 
                      className="space-y-2"
                      initial={{ opacity: 0, x: -20 }}
                      ...
                      <div className="mt-6 bg-white rounded-xl border border-gray-200 overflow-hidden">
                        ...
                        <div className="p-6 space-y-4">
                          <div>
                            <p className="text-sm text-gray-500 mb-1">UUID</p>
                            <p className="text-sm font-mono text-gray-900 break-all bg-gray-50 p-2 rounded">
                              {selectedShipment.blockchain.uuid}
                            </p>
                          </div>
                          <div>
                            <p className="text-sm text-gray-500 mb-1">Кошелек доставки</p>
                            <p className="text-base text-gray-900">
                              {selectedShipment.blockchain.deliveryWallet}
                            </p>
                          </div>
                          <div>
                            <p className="text-sm text-gray-500 mb-1">История статусов</p>
                            <ul className="list-disc list-inside text-gray-900">
                              {selectedShipment.blockchain.statusHistory.map((status, index) => (
                                <li key={index}>{status}</li>
                              ))}
                            </ul>
                          </div>
                          <div>
                            <p className="text-sm text-gray-500 mb-1">История обработчиков</p>
                            <ul className="list-disc list-inside text-gray-900">
                              {selectedShipment.blockchain.handlerHistory.map((handler, index) => (
